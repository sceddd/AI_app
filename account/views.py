...
def get_similar_faces(user,photo_id,k_faces=4):
    ...
    try:
        ...
        for face_idx, face in enumerate(result['faces']):
            ...
            temp = f"0,0.00,{photo_id},{face_idx}"
            result['output'].append(temp)
            for f_id, face_data in enumerate(similar_faces['k_faces']):
                ...
                temp = f"{f_id+1},{distance:.2f},{img_id},{face_idx}"
                result['output'].append(temp)
                print(result)
        if 'faces' in result:
            del result['faces']
...
def predict_objects(photo_id, input_words):
    ...
    try:
        task = process_ob_det.apply_async(args=[photo_id,input_words])
    except photo_class.DoesNotExist:
        return JsonResponse({'error': 'Photo not found'}, status=404)
...
@csrf_exempt
def upload(request):
    logger.info('POST: account/upload')
    ...
    if request.method == 'POST' and 'file' in request.FILES:
        ...
        lmdb_env = lmdb.open(
            str(settings.ZIP_PATH),
            map_size=settings.LMDB_LIMIT * settings.LMDB_BATCH_SIZE,
            max_dbs=0
        )
        try:
            with lmdb_env.begin(write=True) as txn:
                ...
                logger.info(f"Stored zip file with key {zip_key} in LMDB.")
        except Exception as e:
            logger.error(f"Failed to store zip file in LMDB: {e}")
            return JsonResponse({'error': 'Failed to store zip file'}, status=500)
        finally:
            lmdb_env.close()

        # Enqueue the processing task
        task = process_zip_file_lmdb.apply_async(args=[zip_key, user.id], queue="image_upload")
        logger.info(f"Enqueued task {task.id} for zip key {zip_key}.")
        return JsonResponse({'task_id': task.id}, status=202)

    logger.warning('No zip file found in the request.')
